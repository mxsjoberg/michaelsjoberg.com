...
This is an evolving list with learning material and recommended programming languages for computer science students. It works well on its own (for self-learners) or on top of most undergraduate degrees in computer science, which are often lacking various low-level details and mathematical content. The goal is to provide a solid foundation to build upon, degree or no degree.

This list will be updated whenever there is something missing, incorrect, or to add new/ better learning materials (IMO). There is a changelog at the bottom of this page.

If you are also interested in engineering (and have plenty of time to learn), see [github.com/michaelsjoeberg/the-ultimate-computer-science-and-engineering-curriculum](https://github.com/michaelsjoeberg/the-ultimate-computer-science-and-engineering-curriculum).
...
What about object-oriented programming? This is a quote by Paul Graham (Viaweb, Y Combinator): "*[at] big companies, software tends to be written by large (and frequently changing) teams of mediocre programmers. Object-oriented programming imposes a discipline on these programmers that prevents any one of them from doing too much damage*", see [Why Arc Isn't Especially Object-Oriented](http://www.paulgraham.com/noop.html).

### Machine learning

The goal is to get exposed to machine learning and the idea that output is based on data instead of design, watch: [Building the Software 2.0 Stack](https://databricks.com/session/keynote-from-tesla).
...
Mathematics has a central role in any computer science curriculum. A solid foundation in some mathematical topics can be highly valuable, especially in more advanced computing (not to mention ML).
...
This section contains additional topics and commonly used tools, it's somewhat based on the *"missing semester"*, see [The Missing Semester of Your CS Education](https://missing.csail.mit.edu/).
...
Changed order of *Assembly, C/C++, Python* to *Python, C, Assembly* in foundation, to better reflect order of learning.

Moved Python project *game of life* to bonus and removed quantum libraries (lack of relevance).

Removed dead link to Haskell project *building a modern functional compiler from first principles*.

Moved *Coq* to bonus under *Prolog*.
