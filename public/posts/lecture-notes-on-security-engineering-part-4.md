...
After `push f` to increment pointer, create space, and store `f` at address.
...
After `pop %eax` to decrement pointer and store value in `%eax` (`f` is still at `0xbfff7fec` but will be overwritten on next `push`).
...
An example function call and resulting stack layout:
...
Below is a program vulnerable to buffer overflow attack using `nop`-sled and injecting shellcode.
...
An exploit, where shellcode is hexadecimal string encoding machine instructions. The shellcode can also be written in assembly and inserted into executable by compiler, `gcc -o exploit exploit.c shellcode.s`, and `extern char shellcode[];` replacing `unsigned char code[] = ... ;`.
...
Run exploit with `-fno-stack-protector` to disable the stack protection, which is sometimes enabled by default, and `echo 0 | sudo tee /proc/sys/kernel/randomize_va_space` to disable address space layout randomization, ASLR.
